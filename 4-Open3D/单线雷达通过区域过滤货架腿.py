#!/usr/bin/env python3
"""Replay located-lidar rack region filtering from recorded scan point files.

The script mirrors the region branch of modules/laser/laser_filter/rack_filter.hpp:

  BaseRackFilter::filterRack()
    -> filterOutBoarderPoint()
    -> filterScanTrailingPoint()
    -> extractClusters()
    -> RackFilter::findRackCluster()
    -> dilateClusters()
    -> filterRackClusters()

Input files are produced by BasePointCloudProcess::recordPointCloud(LaserScan), so each
line is one original scan index converted to x/y/z in the lidar frame. Zero rows are
preserved and treated as removed/invalid ranges.
"""
...
def recover_scan(points: np.ndarray) -> tuple[np.ndarray, float, float]:
    """Recover ranges, angle_min and angle_increment from recorded xy rows."""
...
def rack_envelope_extra(para: RackRegionPara) -> float:
    if abs(para.rack_wheel_rotate_radius) < 1e-6:
        return 0.03
    return para.laser_std_err + para.rack_wheel_rotate_radius


def rack_rect_extents(para: RackRegionPara) -> tuple[float, float, float, float]:
...
def transform_lidar_to_agv(points_xy: np.ndarray, install: InstallPara) -> np.ndarray:
...
def compute_check_rack_circle(
    angles: np.ndarray, install: InstallPara, para: RackRegionPara
) -> np.ndarray:
    _, max_x, _, max_y = rack_rect_extents(para)
    radius_thresh = math.hypot(max_x, max_y) + para.check_dist_offset

    # C++: laser_tf.inverse() * Eigen::Vector2f(0, 0)
    c = math.cos(-install.laser_coord_yaw)
    s = math.sin(-install.laser_coord_yaw)
    translated = np.array([-install.laser_coord_x, -install.laser_coord_y])
    # 货架中心在雷达坐标系下的坐标
    center_pose = np.array(
        [c * translated[0] - s * translated[1], s * translated[0] + c * translated[1]]
    )
    a0, b0 = center_pose
    r0_sq = a0 * a0 + b0 * b0
    r0 = math.sqrt(r0_sq)
...
def filter_out_border_points(
    ranges: np.ndarray, angles: np.ndarray, install: InstallPara, para: RackRegionPara
) -> np.ndarray:
...
def filter_scan_trailing_points(
    ranges: np.ndarray, points_state: np.ndarray, check_circle: np.ndarray, angle_increment: float
) -> None:
    step = 2
    min_thresh = 0.03
    ...
    for i in range(step, scan_size):
        ...
        dist_1 = math.sqrt(
            max(
                ...
                ranges[i + step] ** 2
                + ranges[i - step] ** 2
                - 2.0 * ranges[i + step] * ranges[i - step] * cos_increment,
            )
        )
...
def extract_clusters(
    ranges: np.ndarray, angles: np.ndarray, points_state: np.ndarray, check_circle: np.ndarray
) -> list[Cluster]:
    clusters: list[Cluster] = []
    curr_indexes: list[int] = []
...
def point_in_rotated_rect(point_agv: np.ndarray, theta: float, rect: tuple[float, float, float, float]) -> bool:
...
def find_rack_clusters(
    ...
    directions = [
        para.rack_rotate + i * para.rectangle_angle_step for i in range(-num_times, num_times + 1)
    ]
...
def dilate_clusters(
    ranges: np.ndarray, rack_clusters: list[Cluster], angle_increment: float, para: RackRegionPara
) -> None:
    ...
    for cluster in rack_clusters:
        ...
        cluster.min_index = dilate_cluster_boarder(
            ranges, angle_increment, cluster.min_index, left_size, -1, para
        )
        right_size = max_index - cluster.max_index if cluster.max_index + delta > max_index else delta
        cluster.max_index = dilate_cluster_boarder(
            ranges, angle_increment, cluster.max_index, right_size, 1, para
        )
...
def apply_located_lidar_post_filters(points: np.ndarray) -> np.ndarray:
...
def replay_region_filter(
    origin_points: np.ndarray, install: InstallPara, para: RackRegionPara
...
def visualize(origin: np.ndarray, filtered: np.ndarray, removed: np.ndarray, install: InstallPara, para: RackRegionPara) -> None:
    import open3d as o3d
    ...
    o3d.visualization.draw_geometries(geoms, window_name="rack region filter replay")
...
def fixed_region_para() -> RackRegionPara:
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--scan-vis",
        action="store_true",
        help="save Cartesian-to-polar recovery visualization as PNG",
    )
    return parser
...
def main() -> int:
    ...
    write_points(FIXED_OUT_DIR / f"{FIXED_SENSOR}_region_replay_filtered.txt", filtered)
    write_points(FIXED_OUT_DIR / f"{FIXED_SENSOR}_region_replay_removed.txt", removed)
    write_points(FIXED_OUT_DIR / f"{FIXED_SENSOR}_region_replay_origin_filter.txt", origin_filter)
    write_points(FIXED_OUT_DIR / f"{FIXED_SENSOR}_region_replay_origin_filter_removed.txt", post_removed)
    ...
    scan_vis_path = FIXED_OUT_DIR / f"{FIXED_SENSOR}_scan_recover_visualization.png"
    if True:
        ...
        print(f"  scan_recover_visualization_png: {scan_vis_path}")
