...
class BOTSORT(BYTETracker):
    ...
    def __init__(self,track_high_thresh,track_low_thresh,new_track_thresh,track_buffer,match_thresh,fuse_score,gmc_method,proximity_thresh,appearance_thresh,with_reid,model):
        ...
        Examples:
            ...
            >>> bot_sort = BOTSORT(args, frame_rate=30)
        """        
        super().__init__(track_high_thresh,track_low_thresh,new_track_thresh,track_buffer,match_thresh,fuse_score)
        ...
        self.encoder = (
            ...
            if self.with_reid and self.model == "auto"
            else ReIDONNX(self.model)
            if self.with_reid
            else None
        )
...
class ReIDONNX:
    def __init__(self, model_path, imgsz=224, use_cuda=True):
        ...
        batch_dim = input_shape[0] if len(input_shape) > 0 else None
        self.dynamic_batch = isinstance(batch_dim, str) or batch_dim is None
        self.fixed_batch_size = batch_dim if isinstance(batch_dim, int) else None
    ...
    def __call__(self, img: np.ndarray, dets: np.ndarray) -> list[np.ndarray]:
        ...
        for det in dets:
            ...
            if x2 <= x1 or y2 <= y1:
                raise ValueError(
                    f"Invalid crop after clipping: "
                    f"(x1={x1}, y1={y1}, x2={x2}, y2={y2}), det={det.tolist()}"
                )

            crop = img[y1:y2, x1:x2]
            if crop is None or crop.size == 0:
                raise ValueError(f"Empty crop generated for det={det.tolist()}")

            crops.append(crop.copy())

        batch = self.preprocess(crops)

        if self.dynamic_batch or self.fixed_batch_size != 1:
            feats = self.session.run([self.output_name], {self.input_name: batch})[0]
        else:
            outputs = []
            for i in range(batch.shape[0]):
                one = batch[i:i + 1]
                feat = self.session.run([self.output_name], {self.input_name: one})[0]
                outputs.append(feat[0])
            feats = np.stack(outputs, axis=0)
