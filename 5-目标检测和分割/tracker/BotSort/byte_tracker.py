...
class BYTETracker:
    ...
    def update(self, results, img: np.ndarray | None = None, feats: np.ndarray | None = None) -> np.ndarray:
        ...
        self.frame_id += 1                  # 当前帧率加1
        activated_stracks = []              # 本帧成功更新、激活的轨迹
        refind_stracks = []                 # 之前丢失，这帧又找回来的轨迹
        lost_stracks = []                   # 本帧判定为丢失的轨迹
        removed_stracks = []                # 本帧判定彻底移除的轨迹

        scores = results.conf               # 取出每个检测框的置信度
        remain_inds = scores >= self.track_high_thresh    # 高分框掩码
        inds_low = scores > self.track_low_thresh         # 低阈值以上掩码
        inds_high = scores < self.track_high_thresh       # 低于高阈值掩码

        inds_second = inds_low & inds_high                     # 在低阈值和高阈值之间的框
        results_second = results[inds_second]                  # 取出这些中等分数框，后面用于第二轮匹配
        results = results[remain_inds]                         # 只保留高分框，作为第一轮匹配输入
        feats_keep = feats_second = img

        detections = self.init_track(results,feats_keep)  # 把当前高分检测结果转成一组STrack对象，放到detections里。每个STrack可以理解成“候选轨迹实例”，里面有框，分数，类别等信息
