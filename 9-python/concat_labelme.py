...
for path in labelmePath:
    ...
    for i,imagepath in enumerate(tqdm(image_path),start=1):
        if not os.path.exists(os.path.join(save_dir,imagepath)):
            ...
            json_name = imagepath.replace('.jpg', '.json')
            if json_name in json_path:
                new_json_name = new_imagepath.replace('.jpg', '.json')
                # shutil.copy2(os.path.join(path, json_name), os.path.join(save_dir, new_json_name))
                json_name = imagepath.replace('.jpg', '.json')
